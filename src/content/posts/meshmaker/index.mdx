...
import newEditor from './new-editor.webm';
...
<video controls autoplay loop muted playsinline>
  <source src={newEditor} type="video/webm" />
</video>
...
Smooth subdivision surfaces are fiddly to get right, and a hobby editor has no business hand-rolling them. MeshMaker never did. It has always leaned on Pixar's [OpenSubdiv](https://github.com/PixarAnimationStudios/OpenSubdiv) for them. The port keeps that dependency, pulled in through CMake's FetchContent, and compiles it to WASM alongside everything else, so the same library now runs in the browser.
