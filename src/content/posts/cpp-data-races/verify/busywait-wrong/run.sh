...
cd "$(dirname "$0")"
bin="$(mktemp)"
# Build the relaxed busy-wait under ThreadSanitizer, then trim its report to the
# stable parts: the conflicting accesses and the file:line they live on.
# Addresses, pids, thread ids and the deep std::thread backtraces are dropped so
# the captured output does not churn between runs.
clang++ -std=c++23 -fsanitize=thread -O1 -g -pthread -I../../code ../../code/busywait.cpp -o "$bin"
TSAN_OPTIONS="halt_on_error=1 exitcode=0" "$bin" wrong 2>&1 | awk '
